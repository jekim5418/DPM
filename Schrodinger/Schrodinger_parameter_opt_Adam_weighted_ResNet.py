...
class PhysicsInformedNN_ResNet:
        ...
        for it in range(nIter):
            ...
            if it % n_outlook == 0:
                self.saver.save(self.sess,
                                    './tf_model/Schrodinger-Adam-weighted-ResNet-%d-%d-%.4f-%d-%d.ckpt' % (
                ...
                if best_val > val_error:
                    best_val = val_error
                    i = it
                    self.saver.save(self.sess,'./tf_model/Schrodinger-Adam-weighted-ResNet-%d-%d-%.4f-%d.ckpt' % (
    ...
    def predict(self, X_star, it):
        ...
        self.saver.restore(self.sess, './tf_model/Schrodinger-Adam-weighted-ResNet-%d-%d-%.4f-%d-%d.ckpt'
                           % (num_layers, num_neurons, learning_rate, weighted, it))
        tf_dict = {self.x0_tf: X_star[:, 0:1], self.t0_tf: X_star[:, 1:2]}
...
def main_loop(num_layers, num_neurons, learning_rate, weighted):
    ...
    model = PhysicsInformedNN_ResNet(x0, u0, v0, tb, X_f, layers, lb, ub, learning_rate, weighted)
    ...
    worst_epoch_of_0, worst_epoch_of_b, worst_epoch_of_f, worst_epoch_of_mape_0, epoch, error_h_extra, validation_error, best_it = \
        model.train(30000, 50, 0.00001, X_val_star, u_val_star, v_val_star, h_val_star, num_layers, num_neurons,
                    learning_rate, weighted)
