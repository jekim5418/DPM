...
class PhysicsInformedNN:
    ...
    def __init__(self, X_u, u, X_f, layers, lb, ub, mu, mu1, mu2, learning_rate, weighted):
        ...
        for it in range(nIter):
            ...
            if it % n_outlook == 0:
                self.saver.save(self.sess,
                                './tf_model/Inviscid_burgers-Adam-weighted-OriginalNN-%d-%d-%.4f-%d-%d.ckpt' % (
                ...
                if best_val > val_error:
                    best_val = val_error
                    i = it
                    self.saver.save(self.sess,'./tf_model/Inviscid_burgers-Adam-weighted-OriginalNN-%d-%d-%.4f-%d.ckpt' % (
    ...
    def predict(self, X_star, it):
        self.saver.restore(self.sess, './tf_model/Inviscid_burgers-Adam-weighted-OriginalNN-%d-%d-%.4f-%d-%d.ckpt' % (
                    num_layers, num_neurons, learning_rate, weighted, it))
        u_star = self.sess.run(self.u_pred, {self.x_u_tf: X_star[:, 0:1], self.t_u_tf: X_star[:, 1:2]})
        f_star = self.sess.run(self.f_pred, {self.x_f_tf: X_star[:, 0:1], self.t_f_tf: X_star[:, 1:2]})
...
def main_loop(num_layers, num_neurons, learning_rate, weighted):
    ...
    model = PhysicsInformedNN(X_u_train, u_train, X_f_train, layers, lb, ub, mu, mu1, mu2, learning_rate, weighted)
