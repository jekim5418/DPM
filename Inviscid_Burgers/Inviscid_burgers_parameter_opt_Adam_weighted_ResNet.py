...
class PhysicsInformedNN_ResNet:
    ...
    def net_u(self, x, t):
        u = self.neural_net(tf.concat([x, t], 1), self.weights, self.biases)  # u를 neural net으로 돌림
        return u
        ...
        for it in range(nIter):
            ...
            if it % n_outlook == 0:
                self.saver.save(self.sess,
                                './tf_model/Inviscid_burgers-Adam-weighted-ResNet-%d-%d-%.4f-%d-%d.ckpt' % (
                ...
                if best_val > val_error:
                    best_val = val_error
                    i = it
                    self.saver.save(self.sess,'./tf_model/Inviscid_burgers-Adam-weighted-ResNet-%d-%d-%.4f-%d.ckpt' % (
    ...
    def predict(self, X_star, it):
        self.saver.restore(self.sess, './tf_model/Inviscid_burgers-Adam-weighted-ResNet-%d-%d-%.4f-%d-%d.ckpt' % (
            num_layers, num_neurons, learning_rate, weighted, it))
        u_star = self.sess.run(self.u_pred, {self.x_u_tf: X_star[:, 0:1], self.t_u_tf: X_star[:, 1:2]})
        f_star = self.sess.run(self.f_pred, {self.x_f_tf: X_star[:, 0:1], self.t_f_tf: X_star[:, 1:2]})
