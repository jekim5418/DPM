...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, layers, lb, ub, learning_rate, weighted):
        ...
        self.loss = (self.loss_0 + self.loss_b)*(1/(1+self.weighted)) + \
                    self.loss_f*(self.weighted/(1+self.weighted))
        ...
        for it in range(nIter):
            ...
            if it % n_outlook == 0:
                self.saver.save(self.sess,
                                './tf_model/AC-Adam-weighted-OriginalNN-%d-%d-%.4f-%d-%d.ckpt' % (
                ...
                if best_val > val_error:
                    best_val = val_error
                    i = it
                    self.saver.save(self.sess,'./tf_model/AC-Adam-weighted-OriginalNN-%d-%d-%.4f-%d-%d.ckpt' % (
                                                            num_layers, num_neurons, learning_rate, weighted, it))
    ...
    def predict(self, X_star, it):
        self.saver.restore(self.sess, './tf_model/AC-Adam-weighted-OriginalNN-%d-%d-%.4f-%d-%d.ckpt'
                                    % (num_layers, num_neurons, learning_rate, weighted, it))
        tf_dict = {self.x0_tf: X_star[:, 0:1], self.t0_tf: X_star[:, 1:2]}
        u_star = self.sess.run(self.u0_pred, tf_dict)
...
def main_loop(num_layers, num_neurons, learning_rate, weighted):
    ...
    model = PhysicsInformedNN(x0, u0, tb, X_f, layers, lb, ub, learning_rate, weighted)
